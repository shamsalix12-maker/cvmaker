...
- **REDESIGN FOR RESILIENCE (Garbage-In Robustness)**: Fundamental pivot in extraction architecture. Replaced rigid TS-side rules with a **Recursive Concept Search** engine that finds CV data anywhere in the AI output tree. This aligns with the core philosophy of "extracting a CV even from garbage," making the system future-proof and structure-agnostic.
- **CONTEXT & ALIAS FIX**: Restored hundreds of key aliases to prevent data loss (fixing the "half CV deleted" issue). Enabled **Domain-Aware Gap Analysis** by injecting the full CV into the assessment phase, ensuring examples are specific to the user's field (e.g., Medicine) and auditing is "forgiving" across sections.
- **ZERO-TRUNCATION POLICY**: Hardened all prompts to strictly forbid summarization. Implemented **Lossless Metadata Capture** that grabs 100% of the AI's JSON output (including unmapped fields like specific dates/institutions) and ensures they are rendered in the final document. Updated the rendering engine to include all CV sections (Teaching, Publications, etc.) in the final output.
