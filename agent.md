...
- **PROJECT RULES (RULES.md)**: Created a central `RULES.md` file to store permanent instructions. Specifically recorded that "Gemini Flash 2.5" is the preferred model for all extractions and must not be confused with other versions.
- **GEMINI 2.5 FLASH PIVOT**: Transitioned the entire software stack to use **Gemini 2.5 Flash** as the default model, ensuring maximum extraction fidelity and adherence to the Zero-Truncation policy.
