# CV Manager Module: Architecture, Logic, and Configuration for AI Optimization

This document consolidates all critical files, logic flows, and improved configurations for the `CV Manager` module. It is designed to be fed into another AI agent to understand the current state, the problems encountered (Dev Mode persistence), and the implemented solutions.

## 1. Core Logic & State Management (`useCV.ts`)
The `useCV` hook is the brain of the module. It handles:
- **Fetching:** Loads CV data. crtical logic added for Dev Mode to prefer Local Storage.
- **Saving:** Persists CV data. Critical logic added to bypass Cloud Save in Dev Mode and write to Local Storage.
- **Extraction:** Orchestrates AI extraction (with client-side mock fallback for Dev Mode).
- **Optimistic Updates:** Ensures UI feels responsive.

## 2. API Layer & AI Extraction (`/api/cv/extract`, `cv-extractor.ts`)
- **Route:** Handles file parsing and validation.
- **Extractor:** Uses `Google Generative AI` (Gemini) to extract structured JSON from raw text.
- **Transformation:** Maps AI output to `ComprehensiveCV` TypeScript interfaces.

## 3. UI Components
- **Page (`page.tsx`):** Main orchestration.
- **Uploader (`CVUploader.tsx`):** Handles file drag/drop and text input.
- **Display (`CVFieldDisplay.tsx`):** Renders the structured data.
- **Manual Entry:** Fallback for manual text input.

## 4. Problem & Solution: Dev Mode Persistence
- **Issue:** In Dev Mode, Postgres RLS policies block saves. The API returns empty/error, causing the frontend to wipe data on refresh or refetch.
- **Solution:**
  - **`saveCV`**: If `isDevUser`, skip API call. Save to `localStorage`.
  - **`fetchCV`**: If `isDevUser`, check `localStorage` FIRST. If data exists, use it and abort the API call.
  - **`extract*`**: Client-side mock fallback if API fails in Dev Mode (guarantees data visibility).

## 5. Key File Contents
See the attached `cv_module_full_context.txt` for the verbatim code of:
- `src/lib/types.ts` (Data Structures)
- `src/hooks/useCV.ts` (State Logic)
- `src/app/[locale]/cv-manager/page.tsx` (Main UI)
- `src/components/cv/CVUploader.tsx` (Input UI)
- `src/lib/cv/cv-extractor.ts` (AI Logic)
- `src/app/api/cv/extract/route.ts` (Backend Endpoint)
- `src/lib/auth/dev-auth.ts` (Dev Auth)

## 6. Optimization Instructions for Next AI
1. **Respect Dev Mode:** Always maintain the checks for `isDevUser` to ensure offline/local persistence works.
2. **Model Selection:** The system currently defaults to `gemini-2.5-flash`.
3. **Prompt Engineering:** The prompts in `cv-extraction-prompt.ts` (not fully included here but referenced) dictate specific JSON schema outputs.
